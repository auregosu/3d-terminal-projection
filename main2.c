...
struct v3 cam = {0, 0, 10};
struct v3 camR = {0, 0, 0};
struct v3 camD = {width*0.5f, height*0.5f, 100};
float camSpeed = 0.05;
float camRSpeed = 0.02;
...
void projectionMatrix(struct v3 i, struct v3 *o, float aspectRatio, float fovTan) {
  ...
  o->x = camD.z*dx/dz + camD.x;
  o->y = camD.z*dy/dz + camD.y;
}
  ...
  while (time >= 0) {
    ...
    for (int i = 0; i < CUB_NUM; i++) {
      for (int j = 0; j < 12; j++) {
        ...
        struct v2 p0 = (v2){triProjected.p[0].x, triProjected.p[0].y};
        struct v2 p1 = (v2){triProjected.p[1].x, triProjected.p[1].y};
        struct v2 p2 = (v2){triProjected.p[2].x, triProjected.p[2].y};
